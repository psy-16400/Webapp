...
def addPerson(t):
    db=getConnection()
    sql='insert into cdbadd values(%s,%s,%s,%s,%s,%s)'
    cr=db.cursor()
    cr.execute(sql,t)
    db.commit()
    db.close()
...
def selectPersonById(id1):
    db=getConnection()
    #sql='select * from cdbadd where Id=%s'
    cr=db.cursor()
    print(id1)
...
def updateVadak(t):
    db=getConnection()
    sql='update kdpadd set FullName=%s,Gender=%s,Age=%s,EmailID=%s,Contact=%s where id=%s'
    cr=db.cursor()
    cr.execute(sql,t)
    db.commit()
    db.close()
